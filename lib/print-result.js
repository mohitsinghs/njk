  ...
  Promise
    ...
    .then(results => {
      ...
      if (opts.watch && !opts.verbose) {
        console.log(chalk`Wrote {yellow ${results.length}} file(s) in ${humanTime(timelog)}`)
      }
      // build mode without verbose
      if (!opts.watch && !opts.verbose) {
        console.log(chalk`\n✨ Wrote {yellow ${results.length}} file(s) in ${humanTime(timelog)}`)
      }
      ...
      if (!opts.watch && opts.verbose) {
        console.log(chalk`{green Rendered successfully.}`)
        results = results
          .map(res => ({
            name: res,
            size: humanSize(res)
        ...
        results
          ...
          .forEach(res => {
            console.log(chalk`  ${res.size} ${' '.repeat(largest - res.size.length)} {cyan ${res.name}}`)
          })
        console.log(chalk`\n✨ Done in ${humanTime(timelog)}`)
      }
    })
}
