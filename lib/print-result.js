  ...
  Promise
    ...
    .then(results => {
      ...
      if (opts.watch && !opts.verbose) {
        console.log(chalk`Wrote {yellow ${results.length}} file(s) in {dim ${humanTime(timelog)}}`)
      }
      // build mode without verbose
      if (!opts.watch && !opts.verbose) {
        console.log(chalk`\n✨ Wrote {yellow ${results.length}} file(s) in {dim ${humanTime(timelog)}}`)
      }
      ...
      if (!opts.watch && opts.verbose) {
        ...
        results
          ...
          .forEach(res => {
            console.log(chalk`  {dim ${res.size}} ${' '.repeat(largest - res.size.length)} {cyan ${res.name}}`)
          })
      }
    })
}
